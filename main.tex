...
We present an extension for microelectrode electrophysiology recordings, primarily in animal models, addressing the unique requirements of intracellular and extracellular recordings.
This extension introduces two datatypes: `icephys' for intracellular and `ecephys' for extracellular recordings, supporting diverse recording modalities from patch-clamp to high-density silicon probes.
Building on existing BIDS principles and prior electrophysiology extensions (EEG, MEG, iEEG), we specify metadata for probes, electrodes, and channels, with particular attention to probe geometry and stereotaxic surgical coordinates essential for reproducibility.
The extension adopts NWB (Neurodata Without Borders) and NIX (Neuroscience Information Exchange) as data formats, bridging the gap between session-level data standardization and study-level organization.
We provide example datasets and demonstrate integration with established tools including SpikeInterface and ProbeInterface.
